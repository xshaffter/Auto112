    ...
    internal class RunData
    {
        ...
        private string Bosses
        {
            get
            {
                ...
                int pageSize = 6;
                int indexFalse;
                var firstFalse = list.Where(boss => !boss.Defeated).FirstOrDefault();
                if (firstFalse != null)
                {
                    indexFalse = list.IndexOf(firstFalse);
                }
                else
                {
                    indexFalse = ((int)Math.Floor(list.Count / 6.0)) * 6;
                }
                ...
                if (end > list.Count)
                {
                    pageSize = list.Count - start;
                }
                var bosses = list.GetRange(start, pageSize);
